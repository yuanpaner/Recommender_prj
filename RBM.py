""" implement RMB
    for sparse data, modified rbm's
    amazon SSTNE also designed for sparse data

    train step:
    Contrastive divergence to use sparse data more efficiently(paper, Gibbs sampler, math behind it)
"""
...
class RBM(object):
    ...
    def __init__(self, visibleDimensions, epochs=20, hiddenDimensions=50, ratingValues=10, learningRate=0.001, batchSize=100):
        """ visibleDimensions: number of movie * distinct rating values (like 5 starts), really big
        """
        self.visibleDimensions = visibleDimensions
        self.epochs = epochs # iteration
        self.hiddenDimensions = hiddenDimensions
        self.ratingValues = ratingValues # 10 cuz 0, 0.5 ... 5
        self.learningRate = learningRate
        self.batchSize = batchSize


    def Train(self, X):
        """ X: 2D array, a row of rating values for every user, rating values are flatten binary  values.
            a row for every user.
            each row contains binary data for the number of movies * 10 possible rating values.
        """
        ops.reset_default_graph()

        self.MakeGraph() # build the graph

        init = tf.global_variables_initializer() #  initializes global variables in the graph
        self.sess = tf.Session() # Launch the graph in a session.
        self.sess.run(init) # Evaluate the tensor `init`.
    ...
    def MakeGraph(self):
        ...
        maxWeight = -4.0 * np.sqrt(6.0 / (self.hiddenDimensions + self.visibleDimensions)) # magnituede of possitive and negative extents of the initial random weight.  from paper
        ...
        hProb0 = tf.nn.sigmoid(tf.matmul(self.X, self.weights) + self.hiddenBias) # output of hidden layer
